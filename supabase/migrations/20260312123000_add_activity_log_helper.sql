...
begin
  if auth.uid() is null and coalesce(v_request_role, '') <> 'service_role' then
...
revoke all on function public.record_organization_activity(uuid, text, text, uuid, jsonb, uuid) from public;
grant execute on function public.record_organization_activity(uuid, text, text, uuid, jsonb, uuid) to authenticated;
grant execute on function public.record_organization_activity(uuid, text, text, uuid, jsonb, uuid) to service_role;
